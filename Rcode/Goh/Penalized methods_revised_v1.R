...
for (rho0 in RHO) {
  ...
  t2.LASSO  <- rep(NA,REP)
  n.var.LASSO  <- rep(NA,REP)
  n.true.LASSO  <- rep(NA,REP)
  ham.LASSO  <- rep(NA,REP)
  
  FDR.SCAD <- rep(NA,REP)
  FOR.SCAD <- rep(NA,REP)
  t2.SCAD  <- rep(NA,REP)
  n.var.SCAD  <- rep(NA,REP)
  n.true.SCAD  <- rep(NA,REP)
  ham.SCAD  <- rep(NA,REP)
  
  FDR.MCP <- rep(NA,REP)
  FOR.MCP <- rep(NA,REP)
  t2.MCP  <- rep(NA,REP)
  n.var.MCP  <- rep(NA,REP)
  n.true.MCP  <- rep(NA,REP)
  ham.MCP  <- rep(NA,REP)
  
  FDR.ENET <- rep(NA,REP)
  FOR.ENET <- rep(NA,REP)
  t2.ENET  <- rep(NA,REP)
  n.var.ENET  <- rep(NA,REP)
  n.true.ENET  <- rep(NA,REP)
  ham.ENET  <- rep(NA,REP)
  ...
  for (k in 1:REP) {
    ...
    choice.LASSO <- which(hat.BETA.LASSO[,which.min(EBIC.LASSO)]!=0)
    ...
    n.true.LASSO[k] <- as.numeric(identical(as.numeric(choice.LASSO),as.numeric(true.var)))
    ###################################
    #2. SCAD ##########################
    ###################################
    t1 <- Sys.time()
    ...
    choice.SCAD <- which(hat.BETA.SCAD[,which.min(EBIC.SCAD)]!=0)
    ...
    n.true.SCAD[k] <- as.numeric(identical(as.numeric(choice.SCAD),as.numeric(true.var)))
    ####################################
    #3. MCP ############################
    ####################################
    t1 <- Sys.time()
    ...
    choice.MCP <- which(hat.BETA.MCP[,which.min(EBIC.MCP)]!=0)
    ...
    n.true.MCP[k] <- as.numeric(identical(as.numeric(choice.MCP),as.numeric(true.var)))
    ###################################
    #4. ENET #########################
    ###################################
    t1 <- Sys.time()
hat.G.ENET<-matrix(0,9,p)
hat.BIC.ENET<-rep(0,9)
alpha0<-seq(0.1,0.9,0.1)
for(goh.i in 1:9){
    fit.ENET<-glmnet(X,y,intercept=FALSE,family="gaussian",alpha=alpha0[goh.i]) #alpha=0.5 ENET, alpha=0 Ridge 
...
choice.ENET <- which(hat.BETA.ENET[,which.min(EBIC.ENET)]!=0)
hat.G.ENET[goh.i,choice.ENET]<-1
hat.BIC.ENET[goh.i]<-min(EBIC.ENET,na.rm = TRUE)
}

    choice.ENET<-which(hat.G.ENET[which.min(hat.BIC.ENET),]!=0)
    ...
    n.true.ENET[k] <- as.numeric(identical(as.numeric(choice.ENET),as.numeric(true.var)))
    print(k)
  }
  ## FDR
  ...
  result0.sd <- apply(result0, FUN = colSds, 3)/sqrt(REP)
  result1 <- cbind(result0.mean[1,],result0.sd[1,],result0.mean[2,],result0.sd[2,],result0.mean[3,],
                   result0.mean[4,],result0.sd[4,],result0.mean[5,],result0.sd[5,],result0.mean[6,],result0.sd[6,])
  result2 <- data.frame(result1, row.names = c('SCAD','MCP','ENET','LASSO'))
  names(result2) <- c('FDR.mean','FDR.se','FOR.mean','FOR.se',
                      'n.true.mean','n.var.mean','n.var.se',
                      'ham.mean','ham.se','time.mean','time.se')
  print(c(p,rho0))
  result.tex <- xtable(result2,digits = 3) 
  print(result.tex)
  # setwd('~/Dropbox/Goh/Project 2/37/FRE')

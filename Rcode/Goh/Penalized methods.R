...
for (rho0 in RHO) {
  ...
  for (k in 1:REP) {
    ...
    ###################################
    t1 <- Sys.time()
    fit.ENET<-glmnet(X,y,intercept=FALSE,family="gaussian",alpha=0.1) #alpha=0.5 ENET, alpha=0 Ridge 
  ...
  result1 <- cbind(result0.mean[1,],result0.sd[1,],result0.mean[2,],result0.sd[2,],result0.mean[3,],result0.sd[3,],

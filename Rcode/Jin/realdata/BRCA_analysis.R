...
##
p.value1 <- as.list(t(p.value))
sml.001 <- which(c(t(p.value))<0.001)
p.value1[sml.001] <- c("<0.001")
...
heat.plot <- ggplot(data.heat, aes(x = Method, y = variable, fill = Coefficient)) + 
  ...
  geom_text(aes(label=unlist(p.value1)), fontface = 'bold', size = 3) +
  theme(panel.background = element_rect(fill='white'))
heat.plot
#ggsave('Heatmap.pdf', plot = heat.plot,dpi = 100, height = 6, width = 6)
ggsave('Heatmap.eps', plot = heat.plot,dpi = 100, height = 6, width = 6)
